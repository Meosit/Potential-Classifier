...
infix fun Double.to(y: Double) = UnclassifiedPoint(this, y)
infix fun UnclassifiedPoint.classifiedBy(classIndex: Int) = Point(this.x, this.y, classIndex)
...
data class PotentialCoefficients(val first: Double, val second: Double, val third: Double, val fourth: Double) {
    ...
    operator fun plus(other: PotentialCoefficients) =
            PotentialCoefficients(first + other.first, second + other.second, third + other.third, fourth + other.fourth)

    operator fun times(c: Double) = PotentialCoefficients(c * first, c * second, c * third, c * fourth)

    fun substitutePoint(point: Point) =
            PotentialCoefficients(first, second * point.x, third * point.y, fourth * point.x * point.y)

    fun getPotentialFunction(): PotentialFunction = { (x, y) ->
...
fun generateDecisionFunction(trainSet: Array<Point>,
                             initialPotentialCoefficients: PotentialCoefficients = INITIAL_COEFFICIENTS
) = generateDecisionFunction(trainSet.asList(), initialPotentialCoefficients)

fun generateDecisionFunction(trainSet: List<Point>,
                             initialPotentialCoefficients: PotentialCoefficients = INITIAL_COEFFICIENTS
): GeneratedFunctions {
